...
namespace ELFVoiceChanger
{
    public static class Disk
    {
        ...
        {
            path = $"{Disk.programFiles}\\{path}.txt";
            ...
            void WriterThread()
            {
                ...
                try
                {
                    using (StreamWriter STR = new StreamWriter(path, savebefore, System.Text.Encoding.UTF8))
                        STR.Write(text);
        ...
        {
            File.Move($"{Disk.programFiles}\\{from}.txt", $"{Disk.programFiles}\\{to}.txt");
        }
        ...
        {
            path = $"{Disk.currentDirectory}\\ProgramFiles\\{path}";
            File.Delete(path);
        }
    }
}
